...
public sealed class CreateBrandSurveyRequestValidator : AbstractValidator<CreateBrandSurveyRequest>
{
    public CreateBrandSurveyRequestValidator()
    {
        RuleFor(x => x.TeamId).NotEmpty();
        RuleFor(x => x.Title).NotEmpty().MaximumLength(200);
        RuleFor(x => x.Description).NotEmpty().MaximumLength(2000);
        RuleFor(x => x.Questions).NotEmpty();
        RuleForEach(x => x.Questions).SetValidator(new CreateSurveyQuestionRequestValidator());
    }

    private sealed class CreateSurveyQuestionRequestValidator : AbstractValidator<CreateSurveyQuestionRequest>
    {
